...
//SQLifeOpenHelper class 는 Database 를 생성하고 해당 Database의 Version을 관리합니다.

// NoteDatabaseHelper 클래스가 정의되어 있다.
// 이 클래스는 SQLiteOpenHelper 클래스를 상속받기 때문에
// 새로 데이터베이스가 만들어져야 하는 상태에서는 onCreate() 메서드가 자동으로 호출된다.
// onCreate()메서드 안에서는 Note  테이블을 만들고 CREATE_DATE 칼럼에 인덱스를 생성
